...
`main.py` を FastAPI でラップし、GitHub にアップロードして、Render などのクラウドサービスにデプロイします。

エージェントへの具体的な依頼文、FastAPI 化の書き換え、requirements.txt の作成、GitHub 連携、Render の Build / Start Command の指定、公開 URL の確認手順は連載 Step 3 で章ごとに書いています。本書で扱うのは「デプロイ後にどう壊れるか」（第 5 章）と「移行ランブック」（第 13 章）の側です。

ブラウザで `https://〇〇.onrender.com/portfolio` を開いて JSON が返れば完了。この URL が Step 4 で AI アプリに登録するエンドポイントになります。

詳細は [連載 Step 3（有料）](https://note.com/wireharbor/n/n4ce581d0a19e)。

## Step 4a: ChatGPT に繋ぐ（GPT Builder）

https://chatgpt.com/gpts/editor を開き、Configure タブの **Actions** セクションに OpenAPI スキーマを貼ります。

curl で実レスポンスを確認してからエージェントにスキーマを書かせる順序、スキーマの書き方、Authentication の設定、Test ボタンでの動作確認、モバイルアプリからの呼び出しまでの具体手順は連載 Step 4a で扱っています。本書で扱うのは「スキーマと実レスポンスがズレたときのエラー」（第 6 章）と「MCP プロトコル内部」（第 10 章）の側です。
